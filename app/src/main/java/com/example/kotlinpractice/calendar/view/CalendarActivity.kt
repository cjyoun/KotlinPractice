...
class CalendarActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.calendar.setOnDateChangeListener { view, year, month, dayOfMonth ->
            Log.d("day num - ", "year : $year / month : $month / dayOfMonth : $dayOfMonth")
            setDayList(year, month, dayOfMonth)
        }
        ...
        binding.setDayBtn.setOnClickListener {
            if(binding.inputDay.text.length != 8){
                ...
                val inputDayYear = inputDay.substring(0 until 4).toInt()
                val inputDayMonth = inputDay.substring(4 until 6).toInt() - 1
                val inputDayDay = inputDay.substring(6).toInt()
                Log.d("inputDay num - ", "year : $inputDayYear / month : $inputDayMonth / dayOfMonth : $inputDayDay")
                setDayList(inputDayYear, inputDayMonth, inputDayDay)
            }
        }
    ...
    // 날짜 계산 함수
    private fun setDayList(year: Int, month: Int, dayOfMonth: Int){
        binding.dayList.text = "" // 초기화
        binding.dDay.text = ""
